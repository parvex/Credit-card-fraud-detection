...
Undersampling wybiera wszystkie rekordy z klasą mniejszościową i dla każdego z nich losowo dobiera wartości z klasą dominującą. Oversampling zaś generuje nowe wartości klasy o mniejszej liczności na podstawie istniejących wartości.

## Undersampling

Zbiór poddany został przekształceniu undersampling dzięki czemu powstał zbalansowany zbiór złożony z wszystkich istniejących wartości klasy stanowiącej mniejszość oraz losowo wybranych rekordów o klasie większościowej.
...
```{r undersampling-cor-boxplot, include=FALSE}
...
```{r oversampling-cor-boxplot, include=FALSE}
# Take variables which have significant correlation with Class. Use oversampled set.
...
Ponownie przeprowadzono przekształcenia undersampling oraz oversampling, tym razem na danych zawierającyh wyłącznie wybrane atrybuty.
...
Następnie, wykonano przekształćenie oversampling.
...
Najpierw przeprowadzono eksperymenty na zbiorach z zastosowaniem techniki undersample, a pózniej techniki oversample. 
W ramach porównania eksperymenty zostały przeprowadzone na pełnym zbiorze oraz na zbiorze zawierajacym wyłącznie wybrane atrybuty.
...
Modele o najlepszych parametrach zostały ostatecznie wyuczone na całym zbiorze trenującym, a ich jakość określona na zbiorze testującym. 
...
log_reg_results <- data.frame(c("Undersample", estimates[1,], estimates[2,], estimates[3,]))
...
log_reg_results <- add_metrics_to_results(metrics, log_reg_results, "Undersample z selekcją atrybutów")

load("rdata/log_req_oversample_final_fit.Rdata")
metrics <- log_req_oversample_final_fit %>% collect_metrics()
log_reg_results <- add_metrics_to_results(metrics, log_reg_results, "Oversample")

load("rdata/log_req_oversample_selected_final_fit.Rdata")
metrics <- log_req_oversample_selected_final_fit %>% collect_metrics()
log_reg_results <- add_metrics_to_results(metrics, log_reg_results, "Oversample z selekcją atrybutów")

log_reg_results
#
```

Patrząc na najważniejsze dla nas kryterium, to znaczy na pole pod krzywą ROC, można zauważyć, że model lepiej radzi sobie w przypadku, gdy został wytrenowany na danych zawierających wszystkie atrybuty. Może to oznaczać, że zostało odrzuconych zbyt dużo atrybutów. Nie widać natomiast wyraźnej różnicy między wynikami dla zbiorów poddanych przekształceniom undersample oraz oversample.
...
estimates = metrics[,".estimate"]
tree_results <- data.frame(c("Undersample", estimates[1,], estimates[2,], estimates[3,]))
...
tree_results <- add_metrics_to_results(metrics, tree_results, "Undersample z selekcją atrybutów")

load("rdata/tree_oversample_final_fit.Rdata")
metrics <- tree_oversample_final_fit %>% collect_metrics()
tree_results <- add_metrics_to_results(metrics, tree_results, "Oversample")

load("rdata/tree_oversample_selected_final_fit.Rdata")
metrics <- tree_oversample_selected_final_fit %>% collect_metrics() 
tree_results <- add_metrics_to_results(metrics, tree_results, "Oversample z selekcją atrybutów")

tree_results
#
```

W przypadku drzewa wyraźnie widać poprawę działania w przypadku zbiorów poddanych "oversample" w stosunku do modeli, które przetwarzały zbiory "undersample". Wskazują na to dokładność oraz f-score. Jednakże, różnice zanikają w przypadku pola pod krzywą ROC. Oznacza to, że model odpowiadający zbiorowi "oversample" wykrywa oszustwa skuteczniej, lecz tylko dla określonego progu użytego do obliczenia dokładności i f-score (0.5). Dla innych progów odcięcia jakość modeli w obydwu przypadkach może być bardziej równa. Warto także zaznaczyć, że dla każdego zbioru wartości zarówno pola pod krzywą ROC jak i pozostałych miar jest niższa niż dla modelu regresji logistycznej, co czyni regresję logistyczną względnie lepszym modelem.

## Sieć neuronowa

Wykorzystano perceptron dwuwarstwowy z 64 neuronami w warstwie ukrytej oraz optymalizator Adam z domyślnymi parametrami (stopa uczenia 0.001, \beta1=0.9, \beta2=0.999). Poniżej przedstawiono wartości miar jakości modelu:
```{r neural network}
load("rdata/neural_net_undersample_final_fit.Rdata")
metrics <- neural_net_undersample_final_fit %>% collect_metrics()
estimates = metrics[,".estimate"]
neural_net_results <- data.frame(c("Undersample", estimates[1,], estimates[2,], estimates[3,]))
...
neural_net_results <- add_metrics_to_results(metrics, neural_net_results, "Undersample z selekcją atrybutów")

load("rdata/neural_net_oversample_final_fit.Rdata")
metrics <- neural_net_oversample_final_fit %>% collect_metrics()
neural_net_results <- add_metrics_to_results(metrics, neural_net_results, "Oversample")
...
neural_net_results <- add_metrics_to_results(metrics, neural_net_results, "Oversample z selekcją atrybutów")

neural_net_results
```

W przypadku zbioru "oversample" uzyskano najlepsze spośród wszystkich modeli dokładność oraz f-score. Są one niezwykle bliskie wartości 1, zarówno w przypadku zbioru z selekcją atrybutów jak i bez. Natomiast, pole pod krzywą ROC ma najniższe do tej pory wartości w przypadku tych zbiorów. Lepiej pod tym względem wypadają zbiory poddane operacji undersample. 
Jeśli chodzi o porównanie z pozostałymi modelami, sieć neuronowa radzi sobie z wykrywaniem oszustw najlepiej spośród wszystkich modeli, o ile wykorzystano zbiory "oversample" oraz próg odcięcia 0.5. Natomiast, krzywa ROC jest najmniej korzystna spośród wszystkich modeli.

...
load("data/tree_oversample_selected_final_fit.Rdata")
tree_oversample_selected_final_fit %>% collect_metrics() # i różne takie trza porobc. Trza ogarnac jak wyluskiwac F-score, accuracy itp.
#
```
...
neural_net_results <- data.frame(c(estimates[1,], estimates[2,], estimates[3,]))
colnames(neural_net_results) <- c("accuracy", "f-score", "roc_auc")
...
add_metrics_to_results(metrics[,".estimate"], neural_net_results)

load("rdata/neural_net_oversample_final_fit.Rdata")
metrics <- neural_net_oversample_selected_final_fit %>% collect_metrics()
add_metrics_to_results(metrics[,".estimate"], neural_net_results)

load("data/neural_net_oversample_selected_final_fit.Rdata")
metrics <- neural_net_oversample_selected_final_fit %>% collect_metrics() 
add_metrics_to_results(metrics[,".estimate"], neural_net_results)
#
```

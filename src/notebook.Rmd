...
```{r training and testing on dataset with all features}
log_reg_tune_spec <- create_log_tune_spec()
tree_tune_spec <- create_tree_tune_spec()
neural_net_spec <- create_neural_net_spec(epochs = 20)
...
#log_reg_workflows <- tune_with_data(log_reg_tune_spec, data_train, list(undersample_recipe))
tree_workflows <- tune_with_data(tree_tune_spec, data_train, list(undersample_recipe))
log_reg_workflows <- list()
neural_net_workflows <- list()

for (recipe in list(undersample_recipe, oversample_recipe)) {
  neural_net_workflows <- append(neural_net_workflows, list(workflow() %>% add_model(neural_net_spec) %>% add_recipe(recipe)))
  log_reg_workflows <- append(log_reg_workflows, list(workflow() %>% add_model(log_reg_tune_spec) %>% add_recipe(recipe)))
}

fit_and_eval(log_reg_workflows[[1]], data_split)
fit_and_eval(log_reg_workflows[[2]], data_split)

fit_and_eval(tree_workflows[[1]], data_split)
#fit_and_eval(tree_workflows[[2]], data_split)

fit_and_eval(neural_net_workflows[[1]], data_split)
fit_and_eval(neural_net_workflows[[2]], data_split)
